...
def test_0():
    ...
    data_product_pepsi = Product(text="Pepsi", value="Pepsi")
    data_model.append(data_product_pepsi)
    data_product_coca = Product(text="Pepsi", value="Pepsi")
    ...
    assert len(data_products_list) == 1
    data_products = data_products_list[0]
    assert data_products[0].data.get("product") == data_product_pepsi
    assert data_products[0].data.get("location") == data_location1
    assert data_products[1].data.get("product") == data_product_pepsi
...
def test_2():
    ...
    assert data_music.data.get("artist") == data_artist
    assert data_music.data.get("music_type") == data_music_type
...
def test_6():
    ...
    assert data_reminders.data.get("date_time") == data_date_time_tomorrow
    assert data_reminders.data.get("person_reminded") == data_contact

    data_events_list = data_model.get_data(EventEntity)
    assert len(data_events_list) == 1
    data_events = data_events_list[0]
    assert data_events.data.get("date_time") == data_date_time_noon
    assert data_events.data.get("location") == data_location
...
def test_7():
    ...
    assert data_orders.data.get("date_time") == data_date_time_22
    assert data_orders.data.get("products") == data_product
    ...
    assert data_reminder.data.get("content") is not None
    assert data_reminder.data.get("content").data.get("value") == data_orders
...
def test_9():
    ...
    assert data_message.data.get("recipient") == data_contact
    assert data_message.data.get("content").data.get("value") == data_weather_forecasts
